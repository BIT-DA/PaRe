...
class GatingNetwork_new2(nn.Module):
    def __init__(self, input_size):
        super(GatingNetwork_new2, self).__init__()
        self.fc1 = nn.Linear(input_size, 64)
        self.act = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        gate_values = torch.sigmoid(self.fc2(self.dropout(self.act(self.fc1(x)))))
        return gate_values
...
class wrapper2D(torch.nn.Module):
    def __init__(self, input_shape, output_shape, use_embedder=True, weight='base', train_epoch=0, activation=None, target_seq_len=None, drop_out=None, from_scratch=False):
        ...
        if self.classification:
            ...
            self.predictor = nn.Linear(in_features=output_dim, out_features=output_shape+10)
    ...
    def __init__(self, input_shape, output_shape, use_embedder=True, weight='roberta', train_epoch=0, activation=None, target_seq_len=512, drop_out=None, from_scratch=False):
        ...
        if not weight == 'swin': 
            ...
            if self.dense:
                ...
                self.predictor = nn.Linear(in_features=768, out_features=output_shape+8)
...
def get_tgt_model(args, root, sample_shape, num_classes, loss, add_loss=False, use_determined=False, context=None, opid=0):
    ...
    if len(sample_shape) == 4:
        ...
        for i, data in enumerate(src_train_loader):
            ...
            for j in range(y_.size(0)):
                l = y_[j]
                filtered_labels = l[(l != 0) & (l != 255)]
                # print(filtered_labels.size())
                max_counts, max_id = torch.mode(filtered_labels, dim=0)
                # print(max_counts.item())
                y_new.append(max_counts.item())
            y_new = torch.tensor(y_new).cuda()
            y_ = y_new
            # y_ = max_counts[y_[:, :, :].numpy()]
            # print(y_.size())
            # print(y_)
    ...
    srcinit = False if args.align_method == 'randinit' else True
