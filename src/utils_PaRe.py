...
from data_loaders_PaRe import load_list
...
class FocalLoss(nn.Module):
    def __init__(self, cls_num=1, cls_num_src=1, gamma=0, alpha=None, size_average=True):
        super(FocalLoss, self).__init__()
        self.gamma = gamma
        self.alpha = alpha
        self.size_average = size_average
        self.cls_num = cls_num
        self.cls_num_src = cls_num_src
        self.criterion = nn.BCEWithLogitsLoss(reduction='mean' if self.size_average else 'sum')
        if torch.cuda.is_available():
             self.criterion =  self.criterion.cuda()

    def forward(self, output, target, lam=1.0, eval=False):
        tar_size = target.size()
        
        if len(tar_size) == 1:
            if eval:
                target = torch.eye(self.cls_num)[target].to(device)
            else:
                target = torch.eye(self.cls_num+self.cls_num_src)[target].to(device)
            ...
            one_hot_labels = torch.zeros((output.size()[0], self.cls_num+self.cls_num_src)).to(device)
            one_hot_labels.scatter_(1, index1.unsqueeze(1), 1-lam)
            one_hot_labels.scatter_(1, index2.unsqueeze(1), lam)
            model_out = F.softmax(output, dim = 1) + 1e-9
            ce = torch.multiply(one_hot_labels, -torch.log(model_out))
            weight = torch.multiply(one_hot_labels, (1 - model_out) ** self.gamma)
            fl = self.alpha * torch.multiply(weight, ce)
            reduced_fl = torch.sum(fl, axis=1)
...
def set_param_grad(model, finetune_method):
    # print('set param grad', finetune_method)
    ...
    elif finetune_method == "emb":
        for n, m in model.named_parameters():
            if 'embed' in n or 'predictor' in n:
                m.requires_grad = True
            else:
                m.requires_grad = False

    elif finetune_method == "stage1":
        for n, m in model.named_parameters():
            if 'embed' in n or 'predictor' in n:
                m.requires_grad = True
            elif 'layers.0' in n or 'layers.1' in n:
                m.requires_grad = True
            elif 'layer.0' in n or 'layer.1' in n: #for language pretrained model
                m.requires_grad = True
            else:
                m.requires_grad = False

    elif finetune_method == "stage2":
        for n, m in model.named_parameters():
            if 'embed' in n or 'predictor' in n:
                m.requires_grad = True
            elif 'layers.2' in n or 'layers.3' in n:
                m.requires_grad = True
            elif 'layer.2' in n or 'layer.3' in n:
                m.requires_grad = True
            else:
                m.requires_grad = False


    elif finetune_method == "stage3":
        for n, m in model.named_parameters():
            if 'embed' in n or 'predictor' in n:
                m.requires_grad = True
            elif 'layers.4' in n or 'layers.5' in n or 'layers.6' in n or 'layers.7' in n or 'layers.8' in n or 'layers.9' in n:
                m.requires_grad = True
            elif 'layer.4' in n or 'layer.5' in n or 'layer.6' in n or 'layer.7' in n or 'layer.8' in n or 'layer.9' in n:
                m.requires_grad = True
            else:
                m.requires_grad = False

    elif finetune_method == "stage4":
        for n, m in model.named_parameters():
            if 'embed' in n or 'predictor' in n:
                m.requires_grad = True
            elif 'layers.10' in n or 'layers.11' in n:
                m.requires_grad = True
            elif 'layer.10' in n or 'layer.11' in n:
                m.requires_grad = True
            else:
                m.requires_grad = False

    elif finetune_method == "stage1&2":
        for n, m in model.named_parameters():
            if 'embed' in n or 'predictor' in n:
                m.requires_grad = True
            elif 'layers.0' in n or 'layers.1' in n or 'layers.2' in n or 'layers.3' in n:
                m.requires_grad = True
            elif 'layer.0' in n or 'layer.1' in n or 'layer.2' in n or 'layer.3' in n:
                m.requires_grad = True
            else:
                m.requires_grad = False

    elif finetune_method.startswith('vit'):
        method = finetune_method.split('-')[-1]
        method_dict = {'l0':['resblocks.0'], 'l0l1':['resblocks.0','resblocks.1'], 'l2l3':['resblocks.2','resblocks.3'],
                       'l8l9':['resblocks.8','resblocks.9'],'l10l11':['resblocks.10','resblocks.11'],'l11':['resblocks.11']}
        for n, m in model.named_parameters():
            if 'embed' in n or 'predictor' in n:
                m.requires_grad = True
            else:
            ...
            for pname in method_dict[method]:
                if pname in n:
                    m.requires_grad = True
...
def embedder_init(source, target, train_embedder=False, match_stats=False):
    if True:
        print('Warning: change initialization strategy to always using source init.')
        ...
        print('Warning: change initialization strategy to using random init.')
