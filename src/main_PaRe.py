...
def main(use_determined, args, info=None, context=None):
    ...
    for ep in range(ep_start, args.epochs + args.predictor_epochs):
        ...
        if ep == args.epochs + args.predictor_epochs - 1:
            ...
            path = 'results_aug/'  + args.dataset +'/' + str(args.align_method) + '_' + str(args.finetune_method)
            print("embedder model save at ", path)
            if not os.path.exists(path):
                os.makedirs(path)
            torch.save(model.cpu().state_dict(), path+'state_dict_ninapro_last.pt')
...
def train_one_epoch(context, args, model, ep, score_func, optimizer, scheduler, loader, src_train_loader, loss, temp, decoder=None, transform=None):    
    ...
    for i, data in enumerate(loader):
        ...
        if args.cls_method == 'multilabel':
            y_src_new = y_src + args.cls_num
            if x.size()[0] == x_src.size()[0]:
                combined_y = torch.stack([y, y_src_new], dim=1)  
        ...
        # if ep % 10 == 0:
        if ep < 10:
            out_raw_mix, lam, flag = mixup_embedding_gate(args, model, out_raw, out_raw_source, ep)
        else:
            out_raw_mix, lam, flag = mixup_embedding_gate_ori(args, model, out_raw, out_raw_source, ep)
        ...
        if args.cls_method == 'multilabel':
            ...
            out_mixed, out_src = model(out_raw_mix)
        ...
        if flag:
            if args.cls_method == 'multilabel':
                l = loss(out, y) + loss(out_mixed, combined_y, lam)
            else:
                if args.dataset == 'NINAPRO':
                    l = loss(out, y, eval=True) + (1-lam) * loss(out_mixed, y, eval=True) + lam * loss_src(out_src, y_src)
                else:
                    l = loss(out, y) + (1-lam) * loss(out_mixed, y) + lam * loss_src(out_src, y_src)
        else:
            if args.cls_method == 'multilabel':
                l = loss(out, y)
            else:
                if args.dataset == 'NINAPRO':
                    l = loss(out, y, eval=True)
                else:
                    l = loss(out, y)
        l.backward()
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                ...
                if n_data >= args.eval_batch_size or i == len(loader) - 1:
                    ...
                    if args.cls_method == 'multilabel':
                        eval_loss += loss(outs, ys, eval=True).item()
                    else:
                        if args.dataset == 'NINAPRO':
                            eval_loss += loss(outs, ys, eval=True).item()
                        else:
